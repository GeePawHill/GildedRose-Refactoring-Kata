...
class GildedRoseTest {
    ...
    @Test
    fun `conjured pre-date items lose quality by 2`() {
        assertBeforeAndAfter(CONJURED_NAME, 10, 5, 9, 3)
    }

    @Test
    fun `conjured pre-date items quality bottoms out at 0`() {
        assertBeforeAndAfter(CONJURED_NAME, 10, 0, 9, 0)
    }

    @Test
    fun `conjured post-date items quality drops by 4 at sellin day`() {
        assertBeforeAndAfter(CONJURED_NAME, 0, 10, -1, 6)
    }

    @Test
    fun `conjured post-date items quality bottoms out at 0 even after sellin day`() {
        assertBeforeAndAfter(CONJURED_NAME, -5, 0, -6, 0)
    }


    fun assertBeforeAndAfter(
        beforeName: String,
        beforeSellin: Int,
        beforeQuality: Int,
        afterSellin: Int,
